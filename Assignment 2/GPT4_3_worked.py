...
def random_search(x_values, y_values, iterations=1000, depth=4,pop=100):
    ...
    for g in tqdm(range(iterations)):
        ...
        #     list_RS.append(candidate_score)
        #     best_tree = candidate_tree
        #     best_score = candidate_score
        # list_RS.append(best_score)
        for i in range(pop):
            candidate_tree = generate_tree(depth)
            candidate_score = mse(candidate_tree, x_values, y_values)
            if candidate_score < best_score:
                ...
                list_RS.append(candidate_score)
            list_RS.append(best_score)
            var.append(candidate_score)
            scatter_plot.append((candidate_score, g))
        var_list_output.append(np.std(var))
        best_tree_expression_list.append(best_tree.__str__())

        batch_best_score.append(best(list_RS))
        # exportFile(best_tree.__str__() , list_RS)
    write_data_to_file(str(batch_best_score),file_path="error_list.txt")
    write_data_to_file(str(best_tree_expression_list),file_path="expression_list.txt")
    write_data_to_file(str(scatter_plot), file_path="scatter_plot.txt")
    write_data_to_file(str(var_list_output), file_path="var_diversity.txt")
    return best_tree,list_RS
...
if __name__ == "__main__":
    ...
    best_tree,listOfError = hill_climbing(x_values, y_values,iterations=iter)
    print(listOfError)
