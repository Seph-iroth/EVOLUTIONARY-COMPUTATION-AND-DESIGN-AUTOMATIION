...
def exportFile(number_list,list_list):
    with open("number_list2.csv", 'w', newline='') as csv_file:
        csv_writer = csv.writer(csv_file)
        for i in number_list:
            csv_writer.writerow([i])
        # csv_writer.writerows(number_list)
    with open("list_list2.csv", 'w', newline='') as csv_file:
...
def main():
    ...
    for times in tqdm(range(iter)):
        ...
        if value_of_the_path < shortest:
            ...
            best_value_tour_path = input_tourList[list_of_length.index(value_of_the_path)]# [[1,2],[3,4]]
